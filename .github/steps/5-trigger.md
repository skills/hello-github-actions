...
**Seeing your _action_ in action**: The status of your the workflow run that's triggered is shown in the pull request before it's merge: look for **All checks have passed** when you try out the steps below. You can also see a list of all or the workflows that are running, or have finished running, in the **Actions** tab of your repository. From there, you can click on each workflow run to view more details and access log files.
